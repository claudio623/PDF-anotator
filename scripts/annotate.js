    ...
    Y.Get.js([
        ...
        function () {
            ...
            var currentcomment, editbox, server, context_quicklist, context_comment, quicklist,
                ...
                nextbutton, resetbutton, prevbutton, choosedrawingtool, findcommentsmenu, stampmenu, resendtimeout, currentpaper, currentline,
            ...
            function resetdio(pageno,submissionid) {//alert(pageno +" "+submissionid);
                 //iki g iso, sek g iso          
                var pagecount, i;
                pageno = parseInt(pageno, 10);
                pagecount = parseInt(server_config.pagecount, 10);
                if ((pageno <= pagecount) && (pageno > 0)) {
                    ...
                    for (i = 0; i < allannotations.length; i += 1) {
                        var yuid = allannotations[i]._yuid;
                        var aid = yuid;
                        console.log('yuid: ' + yuid);
                        console.log('aid: ' + aid);
                        // server.removeannotation(aid);
                        ajax.get('ajax.php', {subsId: submissionid, pageno:pageno}, function() {});
            ...
            function resetcurrentpage() {
                var pageno = parseInt(server.pageno, 10);
                var submissionid = parseInt(server.submissionid, 10);

                pageno += 0;
                resetdio(pageno,submissionid);
            }
            ...
            function startjs() {
                ...
                resetbutton = new Y.Button({ srcNode: '#resetpage' }).render();
                resetbutton.on('click', resetcurrentpage);
